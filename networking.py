...
class PeerConnector:
    ...
    def connect_to_previous_peers(self):
        print(f"[Client] Attempting to connect to previous peers: {[(p['id'], p['host'], p['port']) for p in self.peer_list if p['id'] < self.my_id]}")
            ...
            if peer['id'] < self.my_id:
                try:
                    ...
                    if response and verify_handshake(response, peer['id']):
                        ...
                        print(f"[Client] Handshake FAILED with peer {peer['id']}")
                        sock.close()
                except Exception as e:
                    import traceback
                    traceback.print_exc()
